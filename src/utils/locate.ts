...
export function findColumnWithRow(doc: EmailDocument, columnId: string): { row: Row; col: Column; index: number } | undefined {
...
export function findColumn(doc: EmailDocument, columnId: string): Column | undefined {
...
export function findBlockContext(doc: EmailDocument, blockId: string): { col: Column; index: number } | undefined {
  for (const row of doc.rows) {
    for (const col of row.columns) {
